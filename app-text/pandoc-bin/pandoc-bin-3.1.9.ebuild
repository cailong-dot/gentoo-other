...
SRC_URI="https://mirror.ghproxy.com/https://github.com/jgm/${MY_PN}/releases/download/${PV}/${MY_P}-linux-amd64.tar.gz"
S="${WORKDIR}"/${MY_P}
...
QA_FLAGS_IGNORED="usr/local/bin/${PN}"
...
src_install() {
	...
	newexe bin/${MY_PN} ${PN}
	dosym ${PN} /usr/local/bin/pandoc-lua-bin
	dosym ${PN} /usr/local/bin/pandoc-server-bin
